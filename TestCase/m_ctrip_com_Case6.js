...
// 首页右下角的“我携”
	...
	this.waitForSelector('.tool-box .link-mc',
		function(){
			this.captureSelector(_TimeStamp() + "__112211____" + ".png", '.tool-box .link-mc');
...
function _after_TimeOut(){
	...
	this.capture(_TimeStamp() + "_no_I_waitfor_TimeOut" +".png");
	this.exit();
}

function _after_TimeOut2(_error_point){
	this.echo("\n\n2 Wait for some s, There are timeout, So YOU KNOW ~\n\n");
	this.capture(_TimeStamp() + _error_point + ".png");
	this.exit();
}
...
//	this.capture(_TimeStamp() + "_is_WoXie" +".png");
//	判断是否已经登录
			    	...
			    	function(){
						...
						function(){
							...
							if(this.exists('.cui-bd')){
								...
								this.capture(_TimeStamp() + "ht_wrthwrth" +".png");
   ...
   if(this.exists('.member-box')){
	   ...
	   this.waitForSelector('.sub-viewport .home-i-back',
		...
		this.wait(5000, function(){
			this.capture(_TimeStamp() + "__to_logining" + ".png");
		// 登录页面
		...
		this.waitForSelector('.btn_box .g_btn_s',
			function(){
			   this.capture(_TimeStamp() + ".png");
			   this.captureSelector(_TimeStamp() + "_login_button" + ".png", '.btn_box .g_btn_s');
...
*/
	...
	this.waitForSelector('.sub-city-box',
		function(){
			...
			this.capture(_TimeStamp() + "depart_list" + ".png");
		}, function(){
			this.capture(_TimeStamp() + "_no_City" + ".png");
	...
	// this.captureSelector('departcity.png', 'x(//.main-item/sub-city-box');//*[@data-id=3])');
	   if(!this.exists('.main-item .sub-city-box [data-code="' + _Depart_City + '"]')){
		...
		//	this.capture(_TimeStamp() + "_000__000__000__000__000__000_0" +".png");
           this.exit();
	   }
	...
	this.waitForSelector('.sub-city-box',
		function(){
			...
			this.capture(_TimeStamp() + "arrive_list" + ".png");
		}, function(){
			this.capture(_TimeStamp() + "_no_City" +".png");
	...
	this.then(function(){
	   this.echo("Choose Back City, and then Back ~");
		...
		//	this.capture(_TimeStamp() + "_000__000__000__000__000__000_1" +".png");
           this.exit();
	   }
	...
	this.waitForSelector('.cui_cld_daybox',
		function(){
			...
			this.capture(_TimeStamp() + "_no_monthday" +".png");
			this.exit();
		}, 5000);
...
/*
	...
	this.then(function(){
		...
		this.test.assertExists('.cui_cldunit', "\n\nok, this is calander page，and seleted date: " + _Depart_Date);
		this.captureSelector(_TimeStamp() + "_selected_date" +".png", '.cui_cld_daybox [data-date="' + _Depart_Date + '"]');
		this.test.assertExists('.cui_cld_daybox [data-date="' + _Depart_Date + '"]', "\n\nok, this is culander page ~\n\n");
...
/*	this.wait(5000, function(){
			...
			this.capture(_TimeStamp() + "TTTTTTTTTTTTT" +".png");
			this.capture(_TimeStamp() + "ttttttttttttt" + ".png");
	})
*/
	this.wait(3000);
	...
	this.waitForSelector('#searchlistsubmit',
		function(){
			this.capture(_TimeStamp() + "RRRRRRRRRRRRR" + ".png");
...
function _Choose_O_Flt(){
	...
	this.then(function(){
			this.capture(_TimeStamp() + "Is_there_flight_list" + ".png");
...
//	this.waitForSelector('#addPassenger',
	this.waitForSelector('#js_addPass_btn',
		function(){
			...
			_after_TimeOut2.call(this, "_js_addPass_btn");
		//	this.capture(_TimeStamp() + "__SomeException__" + ".png");
...
//	处理pata 提示框的操作步骤
function _OK_PATA(){
	this.capture(_TimeStamp() + "_some_PATA_or_QTE" + ".png");
...
*/	this.evaluate(function(_zName, _zID){
   ...
   //		$('#linkTel').val("18682089631");
	...
	this.waitFor(
		...
		//	this.capture(_TimeStamp() + "___testtesttesttest01___" + ".png")
			this.evaluate(function(){
...
//	this.capture(_TimeStamp() + "___IVVVVVVVVVVVIII" + ".png");
	...
	*/
	...
	//	this.test.assertSelectorHasText('', '');
		...
		//		this.capture(_TimeStamp() + "___testtesttesttest02___" + ".png")
				this.evaluate(function(){
					$('#linkTel').focus();
			    	$('#linkTel').focusout();
				})
...
function _PsgrInfo_Loss(){
	this.capture(_TimeStamp() + "_passenger_info_Loss" + ".png");
...
function _in_Fill_Passenger(){
	this.waitForSelector('#js_selectPass_btn',
		function(){
			this.echo("go to 1~");
			this.captureSelector( _TimeStamp() + "__AddPassenger__" + ".png", '#js_selectPass_btn');
	...
	this.waitFor(function(){
			this.capture(_TimeStamp() + "__Psgr__" + ".png");
		...
		//	$('[data-id="205359514"]')
			this.capture(_TimeStamp() + "__PassengerList__" + ".png");
	...
	*/
		...
		//	IMPORTANT INFO提示 class="cui-toast" 不能用，但是在这里页面可以使用～
				this.waitForSelector('.cui-toast',
					_PsgrInfo_Loss,
	  				function (){
						this.capture(_TimeStamp() + "__PassengerInfo_ok" + ".png");
						this.echo("oooooooops, Choosed passenger info is complete，continue ~");
					}, 1000);

				this.captureSelector(_TimeStamp() + "__Choosed_Passenger__" + ".png", '[data-id="' + _str + '"]');
	...
	/*		if(this.exists('.rightblue')){
				this.capture(_TimeStamp() + "__PassengerChhoosed__" + ".png");
...
*/
	...
	this.waitForSelector('.rightblue',
		function(){
			...
			this.captureSelector(_TimeStamp() + "__PassengerChoosedBackButton__" + ".png", '.rightblue');
		//	this.capture(_TimeStamp() + "__PassengerChhoosed__" + ".png");
	...
	this.waitForSelector('#passengerInfo',
		function(){
			this.capture(_TimeStamp() + "__PassengerBack2Bill__" + ".png");
...
/*
	...
	//	this.captureSelector(_TimeStamp() + "__PayBox__" + ".png", '#paybox');
		this.captureSelector(_TimeStamp() + "__PayButton__" + ".png", '#paybox .btn');
		...
		function(){
			...
			this.waitForSelector('.cui-toast',
			    ...
			    function oops_no_PATA(){
			    	this.capture(_TimeStamp() + "_noPataorQTE" + ".png");
...
/*
	this.waitForSelector('.cui-layer-padding',
		function(){
			this.captureSelector(_TimeStamp() + "_Fill_orEdit_Error" + ".png", '.cui-layer-padding');
...
function _Result_Shoot(){
	this.wait(3000);
	this.capture(_TimeStamp() + "_afterPAY" + ".png");
...
function _out_Pay_with_Money(){
	...
	this.waitForSelector('#creditcardpay',
		function (){
				...
				this.capture(_TimeStamp() + "_zF00out0" + ".png");
			//	this.captureSelector(_TimeStamp() + "__000000000000000000__" + ".png", '#creditcardpay');
			//	信用卡支付
			...
			//	this.click('#EB_MobileAlipay');
				this.waitForSelector('.list_st_border',
					function (){
						this.capture(_TimeStamp() + "_zF00out1" + ".png");
					//	输入银行卡号
						...
						this.waitForSelector('#li_bankPeriod',
							function (){
								this.capture(_TimeStamp() + "_zF00out2" + ".png");
			...
			this.capture(_TimeStamp() + "_zF00outE" + ".png");
	...
	this.waitForSelector('.paybox #payBtn',
		function(){
			...
			this.capture(_TimeStamp() + "_beforePAY" + ".png");
			this.captureSelector(_TimeStamp() + "__PayPAY__" + ".png", '.paybox #payBtn');
...
function _in_Pay_with_Money(){
	...
	this.waitForSelector('#c_pay_index_lpk',
		function(){
			this.echo("Please pay for ~");
				this.capture(_TimeStamp() + "_zF00in0" + ".png");
				this.captureSelector(_TimeStamp() + "__000000000000000000__" + ".png", '#creditcardpay');
			//	信用卡支付
				this.click('#creditcardpay');
			//	储蓄卡支付
			...
			//	this.click('#EB_MobileAlipay');
				this.waitForSelector('.list_st_border',
					function (){
						this.capture(_TimeStamp() + "_zF00in1" + ".png");
					//	输入银行卡号
						this.evaluate(function(_kaNum){
							document.querySelector('#c_payment_cardbin').value = _kaNum;
						}, '6222651000000009');
						...
						this.waitForSelector('#li_bankPeriod',
							function (){
								this.capture(_TimeStamp() + "_zF00in2" + ".png");
								this.evaluate(function(_ggDate){
									document.querySelector('#bankPeriod').value = _ggDate;
								}, '1216');
							}, function (){
			...
			this.capture(_TimeStamp() + "_zF00inE" + ".png");
...
casper.test.begin("SecondCase", 1, function suite(test){
	...
	casper.start("http://m.ctrip.com/", function() {
		  this.capture(_TimeStamp() + ".png");
	}).then(function(){_TEST_WP('CAN', 'CTU', '2014-09-27')});
	//	登出
...
**  Third Case  **
******************//*
casper.test.begin("ThirdCase", 1, function suite(test){
	// 从这里开始
	casper.start("http://m.ctrip.com/", function() {
		  this.capture(_TimeStamp() + ".png");
...
//  make some change from Xcode, Could i commit it ?
