...
// 首页右下角的“我携”
	...
	this.waitForSelector('.tool-box .link-mc',
		function(){
			this.captureSelector(this._result_cap_str + _TimeStamp() + "__112211____" + ".png", '.tool-box .link-mc');
	...
	this.waitForSelector('.cui_cldunit',
		...
		//	__temp_selector = 'li[data-date="' + _Depart_Date + '"].cui_cld_valid';
			this.waitForSelector('li[data-date="' + _Depart_Date + '"].cui_cld_valid',
				...
				//	this.click('.cui_cld_daybox [data-date="' + _Depart_Date + '"]');
				//	.cui_cld_valid --> 
				//	选择去程日期：	$('li[data-date="2014-10-13"].selected-start')
				//	选择返程日期：	$('li[data-date="2014-10-13"].selected-back')
...
//  It should be controlled in Object this, or there would be some miracle !
	this.this._result_cap_str = "../../TestCapture/Test_SecondCase/";
	// 从这里开始
	casper.start("http://m.ctrip.com/", function() {
		  this.capture(this.this._result_cap_str + _TimeStamp() + ".png");
	...
	casper.run(function (){
        this.echo("No Money to Pay, so Stop here. Bye ~");
    //  this.capture(this._result_cap_str + _TimeStamp() + "_Game_Over" + ".png");		//	casper.exit();
    //  下面这个assertResourceExists方式不应该这么使用，Resource指的是图片、css等东西，而不是某个标签。
        test.assertResourceExists('html',"\n\n牛逼吧？\n\n");
        test.done();
        return 999;
    //	add some tag to check wether teamcity would checkout before building ?
    });
});
