...
**  Second C Case  **
******************/
casper.test.begin("SecondDCase", 1, function suite(test){
//  casper.test.begin("SecondCase", 1, function(test){

	this._result_cap_str = "../../TestCapture/Test_SecondDCase/";
	// 从这里开始
   casper.start("http://m.ctrip.com/", function(){
        this.capture(this._result_cap_str + _TimeStamp() + ".png");
   }).then(function(){_TEST_T.call(this, 'SIA', 'CAN', '2014-10-19')});
   // 登出
   ...
   // 支付
// casper.then(_out_Pay_with_Money4("Credit")).then(_Result_Shoot2);
