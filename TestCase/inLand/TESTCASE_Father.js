...
casper.test.begin("SecondCase", 1, function suite(test){
   ...
   // casper.test.begin("SecondCase", 1, function(test){
	...
	//	支付
	casper.then(_out_Pay_with_Money).then(_Result_Shoot2);
	//	打完收工
	casper.run(function(){
         this.echo("No Money to Pay, 打完收功 用例执行成功. Bye ~");
         this.capture(this._result_cap_str + _TimeStamp() + "_run_Game_Over" + ".png");
		// casper.exit();
