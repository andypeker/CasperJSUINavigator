...
function Choose_Double_Flt2(_flt_, _zc_){
	this.wait(7000, function(){
	//	this.test.assertExists('.sub-city-box',"There are no City List");
		this.echo("stop here waiting for 7s; Show Qu(1) Flight List ~");
		this.capture(this._result_cap_str + _TimeStamp() + ".png");
		this.log("\n*************************************************\n" +
			"*************************************************\n" +
			"Choose 去程 Flight" +
	...
	this.wait(100, function(){
		Flight_Show.call(this);
		_flt_num_ = _flt_  ? _flt_ : 1;
		_zc_num_ = _zc_  ? _zc_ : 1;
	});

	this.wait(200, function(){
		this.log("去程哪个航班呢" + _flt_num_, "info");
		Flight_Select2.call(this, _flt_num_);
	});
	this.wait(2000, function(){
		this.log("去程哪个舱位呢" + _zc_num_, "info");
		ZiCang_Select2.call(this, _zc_num_);
	});

	this.wait(3000, function(){
		this.echo("stop here waiting for 7s; Show Fan(2) Flight List ~");
		this.log("\n*************************************************\n" +
			"*************************************************\n" +
			"Choose 返程 Flight" +
			"\n*************************************************" +
	...
	this.wait(200, function(){Flight_Show.call(this);});

	this.wait(200, function(){
		_flt_num_ = 0;
		this.log("返程哪个航班呢" + _flt_num_, "info");
		Flight_Select2.call(this, _flt_num_);
		// Flight_Select2_BillEdit.call(this, _flt_num_);
	});

	this.wait(2000, function(){
		_zc_num_ = 0;
		this.log("返程哪个舱位呢" + _zc_num_, "info");
		// ZiCang_Select2.call(this, _zc_num_);
		ZiCang_Fan_Select.call(this, _zc_num_);
	});

	this.wait(200, function(){_waiting_ctrip.call(this)});
//	订单填写页检查满仓
//	this.wait(100, function(){Error_before_BillEdit_.call(this)});
//	this.wait(100, function(){_ok_page_BillEdit.call(this)});

	this.wait(100, function(){
		this.log("往返程订单填写:", "info");
		BillEdit_Validate.call(this);
	});
}
