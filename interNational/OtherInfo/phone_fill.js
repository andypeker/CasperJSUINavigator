...
function inter_Fill_Other_Info(){
...
//	this.capture(this._result_cap_str + _TimeStamp() + "___IVVVVVVVVVVVIII" + ".png");
	...
	*/
		this.evaluate(function(_pNum){
			document.querySelector('[data-role="linkTel"]').value = _pNum;
		}, '18682089631');
		//	Test Action
		...
		this.waitFor(
			function(){
				return this.evaluate(function(){
					return document.querySelector('[data-role="linkTel"]').value.length > 10;
				})
			}, function OK_continue(){
		//		this.capture(this._result_cap_str + _TimeStamp() + "___testtesttesttest02___" + ".png")
				this.evaluate(function(){
					$('[data-role="linkTel"]').focus();
			    	$('[data-role="linkTel"]').focusout();
				})
				this.echo("Good News~");
